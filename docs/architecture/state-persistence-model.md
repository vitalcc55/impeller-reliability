...
Persisted: "Project source of truth" {
  ...
  Imports: "imports/r130sh: immutable managed archives"
}
...
WorkerRuntime: "Python worker runtime" {
  ...
  Jobs: "one validation job + one active import job per project session"
...
App-level `health.sqlite` остаётся отдельной инфраструктурной диагностикой. Project truth находится только в `.irproj`: clean pre-release schema v1 хранит metadata, dossier/CaseDocument, immutable document registry, `r130sh_source`, optimistic binding, append-only enrichment resolution, immutable TestExecution/FailureObservation, versioned ReliabilityObservation/ReliabilityDataset и immutable AnalysisInputSnapshot/CalculationSnapshot с audit. Невыпущенные промежуточные schema не мигрируются и не получают compatibility layer.
...
Расчёт РБД сохраняет выбранные входы и результат одной атомарной транзакцией вместе с audit. Повтор той же операции определяется по caller IDs и содержимому без новой записи; новый выбор создаёт новую immutable пару. Reopen ограниченно проверяет структуру, связи и сохранность снимков, но не пересчитывает старый результат новым алгоритмом. Renderer draft и ожидание ответа не становятся persisted truth до подтверждённой записи.
