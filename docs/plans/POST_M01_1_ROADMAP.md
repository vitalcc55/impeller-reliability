...
Первый настоящий продуктовый workflow: создать/открыть каталог `.irproj`, управлять одним writer, выполнять forward-only migrations с backup, хранить manifest, минимальные Project/Customer/WheelModel/Specimen/TestCampaign/SourceDocument и append-only audit, затем повторно открыть проект. `health.sqlite` остаётся app-level диагностикой и не превращается в `project.sqlite`.
