...
- Первый расчёт РБД по ПМИ Р130У: инженер выбирает неизменяемый импорт и редакцию плана, явно задаёт происхождение входов и получает точные параметры испытания отдельно от уставок R130SH и фактической наработки. Входной и результатный снимки сохраняются атомарно, доступны в истории после повторного открытия дела; формула таблицы 3 применяется только при документированном точном времени и поддерживаемой структуре испытания. Остаточные дефекты редактора M04B исправлены без изменения прежних версий.
